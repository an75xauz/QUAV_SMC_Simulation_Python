...
def load_model(actor_path, device, state_dim=12, action_dim=6, hidden_sizes=[256, 256]):
    """
    載入訓練好的 Actor 模型
...
def main():
    ...
    parser.add_argument(
        '--model_dir', type=str, default='logs',
        help='模型存放目錄 (預設: logs)'
    )
    parser.add_argument(
    ...
    while not done:
        ...
        # 將參數應用到控制器
        controller.lambda_alt = smc_params[0]
        controller.eta_alt = smc_params[1]
        controller.lambda_att = smc_params[2]
        controller.eta_att = smc_params[3]
        controller.lambda_pos = smc_params[4]
        controller.eta_pos = smc_params[5]
